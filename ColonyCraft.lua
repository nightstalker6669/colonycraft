...
function scanWorkRequests(mon, rs, chest)
    ...
    for w in pairs(workRequests) do
        ...
        useME = 1
        if string.find(desc, "Tool of class") then useME = 0 end
        ...
        if string.find(name, "Bow") then useME = 0 end
        if string.find(name, "Sword") then useME = 0 end
        if string.find(name, "Shield") then useME = 0 end
        if string.find(name, "Helmet") then useME = 0 end
        if string.find(name, "Leather Cap") then useME = 0 end
        if string.find(name, "Chestplate") then useME = 0 end
        if string.find(name, "Tunic") then useME = 0 end
        if string.find(name, "Pants") then useME = 0 end
        if string.find(name, "Leggings") then useME = 0 end
        if string.find(name, "Boots") then useME = 0 end
        if name == "Rallying Banner" then useME = 0 end
        ...
        color = colors.blue
        if useME == 1 then
           -- print("attempting to craft:"..item)
            itemget = bridge.getItem({name = item})
                if itemget ~= nil and itemget.amount ~= nil then
                    print(itemget.name.. " " ..itemget.amount.. "/"..needed)
                    if itemget.amount >= needed then
                        print("we have enough, exporting: "..item)
                        bridge.exportItem({name=itemget.name, count=needed}, chest)
                        color = colors.green
                    end
                    if itemget.amount < needed or itemget.amount == nil then
                        if bridge.isItemCrafting( { name = itemget.name } ) then
                            print(itemget.name.. " is currently crafting")
                            color = colors.yellow
                            print("[Crafting] ", itemget.name)
                        else
                        if itemget.isCraftable then
                            color = colors.yellow
                            print(itemget.name.. " is craftable, starting crafting.")
                            print("[Scheduled]", needed, "x", itemget.name)
                            bridge.craftItem({name = itemget.name, count = needed - itemget.amount})
                        else
                        color = colors.red
                        print("[Failed]", itemget.name)
                        end
                    end
                end
        end
